...
for eid = 1:length(g.edges)
  ...
  if (strcmp(edge.type, 'P') != 0)  % pose-pose
    ...
    % measurement and the information matrix respectively.  % edge is the edge id
    Zij = v2t(edge.measurement);
    eij= t2v(inv(Zij) * (inv(x1) * x2)) ;
    Fx += eij * edge.information * eij';

  % pose-landmark constraint
  elseif (strcmp(edge.type, 'L') != 0) % land marks
